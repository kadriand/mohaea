...
public class ElitistMOHAEA<T extends Individual> extends Algorithm<T> {

    public ElitistMOHAEA(List<GeneticOperator<T>> geneticOperators, TerminationCondition<T> terminationCondition, SelectionMethod<T> selectionMethod, PopulationInitialization<T> initialization, FitnessCalculation<T> fitnessCalculation) {
    ...
    public Population<T> apply() {
        ...
        while (terminationCondition.keepIteratingCondition(iteration, best, bestBefore)) {
            ...
            Population<T> newPopulation = new Population<>(parentsAndChildren);
            newPopulation.sort(Comparator.comparing(T::getFitness));

            int size = initialization.getSize();
            for (int i = 0; i < size; i++)
                newPopulation.remove(size);
            ...
            for (Map.Entry<T, Population<T>> offspring : individualOffsprings.entrySet()) {
                ...
                if (!newPopulation.contains(parent))
                    operatorsRates.remove(parent);

                for (T child : offspring.getValue()) {
                    if (!newPopulation.contains(child))
                        continue;
                    operatorsRates.put(child, parentOperatorsRates.clone());
                }
            }
